...
import base64
class mailfunctions:
	def getlabeledemail(service, labels=["INBOX", "UNREAD"]):
		'''
	...
	def sendmessage(service, message):
		...
		for i in range(0, 3):
			try:
				...
				#print("Opsi woopsi") #This was ust for fun when attempting to debug some issue.
				time.sleep(2)
	...
	def getemailaddr(emailusertext):
		...
		if ("<") in emailusertext and (">") in emailusertext:
